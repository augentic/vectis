...
impl ServerSentEvents {
    /// Subscribe to a server-sent event stream. Each SSE message is delivered
    /// as an [`SseMessage`] containing the event name and raw data bytes.
        ...
        Event: Send + 'static,
    {
        ...
        StreamBuilder::new(|ctx| -> Pin<Box<dyn Stream<Item = SseMessage> + Send>> {
